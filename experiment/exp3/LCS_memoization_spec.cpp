...
private:
    int LCSLength_impl(int i, int j) {
        if (i == 0 || j == 0) {
            calculator[i][j] = 0;
            return 0;
        }
        if (calculator[i][j] != -1) return calculator[i][j];
        else {
            if (x[i - 1] == y[j - 1]) { // 由c[i-1][j-1]得到c[i][j]
                calculator[i][j] = LCSLength_impl(i - 1, j - 1) + 1;
            }
            else if (LCSLength_impl(i - 1, j) >= LCSLength_impl(i, j - 1)) {    // 由c[i-1][j]得到c[i][j]
                calculator[i][j] = LCSLength_impl(i - 1, j);
            }
            else {  // 由c[i][j-1]得到c[i][j]
                calculator[i][j] = LCSLength_impl(i, j - 1);
            }
        }
    ...
    void LCSLength() {  // 动态规划的核心代码
        ...
        for (auto&& vec : calculator) {
            vec.resize(y.size() + 1, -1);
        }
        LCSLength_impl(x.size(), y.size());
    ...
    void CLCS() {
        ...
        while (i && j) {
            ...
            if (x[i - 1] == y[j - 1]) {  // 如果满足该条件，表示是最有解中的一个元素
                result.push_front(x[i - 1]);    // 由于是逆序查找的，所以向前插入
                --i;
                --j;
            }
            else if (calculator[i - 1][j] >= calculator[i][j - 1]) {    // 向上查找
                --i;
            }
            else { // 向左查找
                --j;
